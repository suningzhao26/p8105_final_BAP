...
b2003 = read_xpt("./data/brfss/cdbrfs03.xpt") %>% 
  janitor::clean_names() %>% 
  filter(state == 36) %>% 
  select(state_code = state, county_code = ctycode, year = iyear, month = imonth, day = iday, physhlth, menthlth, cvdcrhd2, cvdinfr2, cvdstrk2, asthma2, asthnow, educag,sex, smoker2, asthmage,asattack,asymptom, asthmst, ltasthm, casthma, race2, ageg5yr, incomg) %>% 
  drop_na(county_code) %>% 
  mutate(
    physhlth =  revalue(physhlth, "88" = "0")
  )
